...
void handle_b_type_instruction(uint32_t instruction) {
    ...
    uint32_t opcode = instruction & 0x7F;
    switch (funct3) {
        case 0x0: if ((int32_t)registers[rs1] == (int32_t)registers[rs2])    //BEQ (Branch if equal)
                   pc = pc + (imm<<1);
                   else
                   pc = pc + 4;
                   break;          
        case 0x1: if ((int32_t)registers[rs1] != (int32_t)registers[rs2])    //BNE  (Branch if not equal)
                   pc = pc + (imm<<1);
                   else
                   pc = pc + 4;
                   break;          
        case 0x4:  if ((int32_t)registers[rs1] < (int32_t)registers[rs2])    //BLT (Branch if less than)
                   pc = pc + (imm<<1);
                   else
                   pc = pc + 4;
                   break;   
        case 0x5: if((int32_t)registers[rs1] >= (int32_t)registers[rs2])    //BGE (Branch if greater than or equal)
                   pc = pc + (imm<<1);   
                   else
                   pc = pc + 4;
                   break;
        case 0x6: if(registers[rs1] < registers[rs2])  //BLTU(Branch if less than unsigned)
                   pc = pc + (imm<<1);
                   else
                   pc = pc + 4;
                   break;   
        case 0x7: if(registers[rs1] >= registers[rs2])  //BGEU(Branch if greater than or equal unsigned)
                   pc = pc + (imm<<1);
                   else
                   pc = pc + 4;
                   break;    
